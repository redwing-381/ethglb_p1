...
function cleanMarkdown(text: string): string {
  return text
    ...
    .replace(/^[-*]\s+/gm, '• ')       // bullet points → clean bullets
...
export function DebateResults({ debate, costBreakdown, isLoading, error }: DebateResultsProps) {
  ...
  if (error) {
    return (
      <AnimatedCard>
        <Card className="border-destructive/50 bg-destructive/5 border-0 shadow-none">
  ...
  return (
    <AnimatedCard className="relative overflow-hidden">
      ...
      <Card className="border-0 shadow-none max-h-[80vh] flex flex-col">
